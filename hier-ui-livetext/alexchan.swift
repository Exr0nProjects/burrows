#!/usr/bin/env swift
/// Originally from https://github.com/alexwlchan/scripts/blob/main/macos/get_live_text
/// Modified by @exr0n to also output detected coordinates
...
func recognizeTextHandler(request: VNRequest, error: Error?) {
  ...
  let recognizedStrings = observations.compactMap { observation in
    ...
    //return ([(0.0, 0.0), (0.0, 0.0), (2.2, 2.2), (3.3, 3.3) ], observation.topCandidates(1).first?.string)
    return [
        "p": corners.compactMap { p in [p.x, p.y] },
        "t": observation.topCandidates(1).first?.string
    ]
    //return 0.0
  }
